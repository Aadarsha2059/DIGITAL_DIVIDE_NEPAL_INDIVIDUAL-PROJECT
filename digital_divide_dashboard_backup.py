    ...
    try:
        if chart_type == "Advanced Bar Chart":
            ...
            if len(metrics) == 1:
                metric = metrics[0]
                if year:
                    plot_data = data.groupby(['District', 'Urban_Rural'])[metric].mean().reset_index()_index()
                    fig = px.bar(plot_data, y='District', x=metric, color='Urban_Rural',
                               title=f"{metric.replace('_', ' ').title()} by District ({year})",
                               color_discrete_sequence=['#FF6B6B', '#4ECDC4'],
                               orientation='h')
...
moji)
    on
    """
    district_data = df_combined[df_combined['District'] == di
    ...
    # Calculate composite score based on key mecs
    avg_internet = latest_data['Internet_Access_Rate'])
    ...
    # Weighted composite sc0)
    composite_score = (
    ortant
        avg_electricity * 0.30ture
        avg_literacy * 0.20 +       # Literacy enables digital adop
...
def generate_prescriptive_recommendations(df_combined, district):
    ...
    latest_year = distric
    latest_data = district_data[district_data['Year'] == latest_year]ar'].max()t_data['Yeicttr] == disd['District'df_combinebined[m""ta analysise damprehensiv on coseddations bave recommenptirirate prescand accute sharp "Genera""", "🟢")re < 80:scoposite_lif come"🟡edium", iority-m, "pre"", "🟠")high "priority-",edscore < 50:site_ompoc)ucturestrtionuccal infrastrti is critricity# Elec +     imp is mostetrn     # Intet * 0.35 + erne_int    avg (0-10ore.mean(ritlates'] == a['Yeartrict_datisa[dt_dat districdata =t_eslatstrict]e Projectiearwistion and Yt Allocacross Budge uniform anditions areCo_econditiontion_class, l, condin_labe(conditiotuple ns:     Retur
...
def main():
    ...
    with st.sidebar:
        ...
        # District selection with enhanced styling
        ...
        # Analysis type with better organization
        st.markdown("### 📊 Analysis Modules")
        analysis_type = st.radio(
            "Choose Analysis Type:",
            ["📋 Overview", "⚖️ Comparative Analysis", "🎨 Custom Visualizations", 
             "💰 Budget Allocation", "📅 Yearwise Projection", "🔮 Predictive Modeling", 
             "💡 Prescriptive Recommendations", "📥 Data Downloads"],
            help="Select the type of analysis you want to perform"
        )
        
        # Clean up analysis type for processing
        analysis_type = analysis_type.split(" ", 1)[1] if " " in analysis_type else analysis_type
        ...
        # Add sidebar footer with quick stats
        st.markdown("---")
        ...
        if df_combined is not None and not df_combined.empty:
            ...
            with col1:
                ...
                st.metric("Years", len(df_combined['Year'].unique()))
            with col2:
    ...
    if analysis_type == "Overview":
        ...
        st.markdown('<h2 class="sub-header">📅 Yearwise Projection Analysis</h2>', unsafe_allow_html=True)
        
        st.markdown("""
        <div class="budget-card">
            <h3>📊 Future Trends & Projections</h3>
            <p>Analyze historical trends and project future development across all districts based on selected metrics.</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Metric selection for projection
        st.markdown("### 🎯 Select Metrics for Projection")
        
        col1, col2 = st.columns(2)
        
        with col1:
            projection_metrics = st.multiselect(
                "Choose metrics to analyze:",
                ['Internet_Access_Rate', 'Electricity_Access_Rate', 'Telephone_Access_Rate', 
                 'TV_Access_Rate', 'Radio_Access_Rate', 'Literacy_Rate_Total'],
                default=['Internet_Access_Rate', 'Electricity_Access_Rate'],
                help="Select one or more metrics for yearwise projection analysis"
            )
        ...
        with col2:
            projection_years = st.slider(
                ...
                max_value=10,
                value=5,
                help="Number of years to project forward from the latest data"
            )
        ...
        if projection_metrics:
            ...
            # Calculate projections for all districts
            st.markdown("### 📈 District-wise Projections")
            
            # Create tabs for different views
            tab1, tab2, tab3 = st.tabs(["📊 Overview Table", "📈 Projection Charts", "🎯 District Details"])
            
            with tab1:
                st.markdown("#### 📋 Comprehensive Projection Overview")
                
                # Build projection summary table
                projection_data = []
                ...
                for district in all_districts:
                    ...
                    district_info['Condition'] = f"{condition_emoji} {condition_label}"
                    ...
                    # Calculate current values and projections for each metric
                    for metric in projection_metrics:
                        if metric in latest_data.columns:
                            current_value = latest_data[metric].mean()
                            district_info[f'{metric}_Current'] = f"{current_value:.1f}%"
                            
                            # Get projection
                            future_years, predictions, r2 = predict_future_trends(
                                df_combined, district, metric, projection_years
                            )
                            
                            if predictions is not None and len(predictions) > 0:
                                projected_value = predictions[-1]
                                growth = projected_value - current_value
                                district_info[f'{metric}_Projected'] = f"{projected_value:.1f}%"
                                district_info[f'{metric}_Growth'] = f"{growth:+.1f}%"
                            else:
                                district_info[f'{metric}_Projected'] = "N/A"
                                district_info[f'{metric}_Growth'] = "N/A"
                    
                    projection_data.append(district_info)
                
                # Display as dataframe
                projection_df = pd.DataFrame(projection_data)
                st.dataframe(projection_df, use_container_width=True, height=400)
                
                # Download option
                st.markdown("#### 📥 Download Projection Data")
                csv = projection_df.to_csv(index=False)
                st.download_button(
                    label="📥 Download as CSV",
                    data=csv,
                    file_name=f"yearwise_projection_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv"
                )
            
            with tab2:
                st.markdown("#### 📊 Visual Projection Analysis")
                
                # Allow user to select districts for detailed charts
                selected_districts_proj = st.multiselect(
                    "Select districts to visualize (max 6 for clarity):",
                    all_districts,
                    default=all_districts[:3],
                    help="Choose up to 6 districts for detailed projection charts"
                )
                
                if selected_districts_proj:
                    # Create projection charts for each metric
                    for metric in projection_metrics:
                        ...
                        for i, district in enumerate(selected_districts_proj[:6]):
                            # Historical data
                            district_data = df_combined[df_combined['District'] == district]
                            historical = district_data.groupby('Year')[metric].mean().reset_index()
        ...
        olist())
                        ...
                        if all_values:
                            ...
                            lowest_val = min(all_values)
                            
        
                  e
                            lowest_district = None
                            for district in selected_districts_proj[:6]:
                                district_data = df_combin]
                                if not dis
        
                                 
                                    if lowest_val in district_d:
        
                        
                        fig.update_layout(
                            title=f"{metric.replace('_', ' ').title()} - Historical & Projected Trends",
                            xaxis_title="Year",
                            yaxis_title=f"{metric.replace('_', ' ').title()} (%)",
                            hovermode='x unified',
                            template='plotly_white',
    ...
    left",
                        ...
                        # Display highest and lowest metrics
        ...
        l_b:
            ...
            with tab3:
                st.markdown("#### 🎯 Detailed District Analysis")
                
                # District selectoiew
            
                    "Select distric,
                    all_districts,
                    help="Choose a district to see comprehensive metric analysis"
                )
                
                if selected_detailct:
            ata
                    district_data =ct]
                    
                    # Get district condition
                    condition_label, condition_class, condition_emoji = get_district_condition(
                        df_combined, selected_detail_district
            )
                    
                    st.markdown(f"### {condition_emoji} {sabel}")
               
                    # Create multi-mchart
                    fig = go.Figure()
                    
                    colors_detail = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD']
                    
                    for i, metric in enumerate(projection_metrics):
                        if metric mns:
                ndex()
                            ...
                            fig.add_trace(go.Scatter(
                                ...
                                mode='linesrkers',
                                name=metric.replace('_', ' ').title(),
                                line=dict(color=colors_detail[i % len(,
                                marker=dict(size=10)
                            ))
        ...
        rue)
                    ...
                    summary_data = []
                    for metretrics:
                        if metric in district_ds:
                            late
                            latest_value = distrc].mean()
                            
                            # Calculate year
                            first_year = d)
                            first_value = district_.mean()
                            growth = latest_valulue
                            
                            # Get projection
                            future_years, predictions, r2 = predtrends(
                                df_combined, selected_detaears
                            )
                            
                            if predictions is no > 0:
        ...
        (),
                        ...
                        metric_values.sort(key=lambda x: x['val
             
        1):
                            st.success(f"{i}. {item['metric']}: {item['value']:.1f}%")
           ...
           1):
            ...
            st.info("Please select at least one metric to as.")
    
    ns":
        st.markdown('<ml=True)allow_htafe_ unstions</h2>',mendaive Recom💡 Prescripteader">"sub-h2 class=hommendatioriptive Rec"Presctype == nalysis_if ael projectionyearwisenalyze f}%"):.1tem['value']etric']}: {i. {item['mi}t.error(f"{    s]), lues[-3:ric_vaed(meterate(reversumitem in en   for i,              ment**")ing Improve Needasth col2:   wi             ], alues[:3ate(metric_vnumerin efor i, item                            True), reverse=ue']ric].mt_data[metlue': lates    'va .titlee('_', ' ').replacmetric 'metric':                            ar]test_yeear'] == laata['Ya[district_district_datta = d  latest_da   .max(']ta['Year_da")Metrics**t rongeswn("**🔝 St.markdo        wit              lightsighmance H Perfor### 📈"#rkdown(this discs for owest metri lnd Highest a         _width=Terontain_cmary_df, use(sumt.dataframe        s  )"N/A"se " elAth != "N/future_growif +.1f}%" wth:"{future_gro frowth':"N else  != "N/A"_valuerojected" if p:.1f}%ed_valueectf"{projtion': ecars} Projyerojection__year + p  f'{latest  .1f}%","{growth:+owth': fl Grue:.1f}%",est_val{latf": }%",1flue:."{first_va} Value': ffirst_year, ' ').teplace('_'c': metric.rri        'Met                "th = "N/Aowuture_gr        f                /A"value = "N_e:els                    aluest_ve - lateected_valuh = projure_growt    fut                   -1]s[rediction_value = pcted   proje                ctions) len(predit None and_yectionproj metric, rict,l_distiure_ict_fute - first_vatric]_year][me== firstr'] eaata['Y_ddistrictata[d'Year'].min(trict_data[isst to last m firwth frogroear][metriest_year'] == latict_data['Ystr_data[diict].max()ear'_data['Y= districtst_year mncoluata.n_m projectioic indth=Twiainer_ig, use_contrt(fplotly_cha         st.   t=50       heigh         ict}"ail_distrd_dette {seleccs Trend -update_lig.    fth=3)il)], widcolors_deta+maetar'],torical['Ye     x=his           eset_imean().r].[metricr')upby('Yea.grotrict_datadisical =    histor         oluata.crict_distin drison c compaetri     dition_l} - {conictetail_distrelected_d        riail_distted_detselec== strict'] ined['Died[df_combcombin df_strict dGet di      #   _distrinalysis:" detailed at for(t.selectboxct = sstri_detail_di   selected tailed v for derojections.prsualize vitrict to is dast onect at lese selean("---arkdow     st.m       1f}%lowest_val:. {_district} -*: {lowestt*f"📉 **Lowest.error(    s            with co                  :.1f}%")al {highest_vrict} -ighest_dist*: {h*Highest*ess(f"🔝 *.succwith col                    (2)columnsol_b = st.lues:l_va  if al        idth=True)ntainer_w use_cofig,t(              hor="anc x                           ",="topyanchorv",ntation="orie                        nd=dict    lege               ictistristrict = dlowest_d                                c].values[metriataictdistrdistrict =   highest_     ues:alta[metric].vrict_da_val in dist if highest                           pty:t_data.emrictict= distrrict'] =['Distbinedomed[df_c = Non_districtighest   h       sest valueowghest and lwith hid districts     # Fin                      .tic][metrt_datatrictend(disll_values.ex          a              .emct_data distridistric == t']icstred['Dif_combinined[d df_combdata =rict_     dist               prodistricts_
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown(f"### 🎯 Recommendations for {district1}")
            recommendations1 = generate_prescriptive_recommendations(df_combined, district1)
            for i, rec in enumerate(recommendations1, 1):
                st.markdown(f"{i}. {rec}")
        
        with col2:
            st.markdown(f"### 🎯 Recommendations for {district2}")
            recommendations2 = generate_prescriptive_recommendations(df_combined, district2)
            for i, rec in enumerate(recommendations2, 1):
                st.markdown(f"{i}. {rec}")
        
        # General recommendations
        st.markdown("### 🌟 General Strategic Recommendations")
        
        general_recs = [
            "🏗️ **Infrastructure Development**: Prioritize fiber optic network expansion in rural areas with low connectivity.",
            "📱 **Mobile-First Strategy**: Leverage high mobile penetration for internet access through 4G/5G networks.",
            "🎓 **Digital Literacy Programs**: Implement comprehensive digital skills training alongside infrastructure development.",
            "🤝 **Public-Private Partnerships**: Collaborate with telecom companies for sustainable connectivity solutions.",
            "💰 **Subsidized Access**: Provide affordable internet packages for low-income households.",
            "🏫 **Educational Integration**: Establish digital learning centers in schools and community buildings.",
            "📊 **Continuous Monitoring**: Implement regular data collection to track progress and adjust strategies."
        ]
